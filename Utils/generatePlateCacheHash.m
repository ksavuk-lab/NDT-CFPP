function hashStr = generatePlateCacheHash(plateCacheKey)
% GENERATEPLATECACHEHASH - Generate a unique hash string for plate cache identification
%
% This function creates a unique hash string based on all parameters that affect
% plate generation results, ensuring that cached data is only used when all
% relevant parameters match exactly.
%
% Input:
%   plateCacheKey - Structure containing all parameters affecting plate generation
%
% Output:
...
try
    ...
    keyStr = struct2str(plateCacheKey);
    ...
    fprintf('Warning: Plate hash generation failed (%s). Using timestamp fallback.\n', ME.message);
